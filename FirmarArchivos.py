...
configure_logger('loggerfirm.log')

def firmar(validador, pdf, firma, contra, pagina, pre_x0, pre_y0, pre_x1, newPDF, company, dni, i=0):
    ...
    try:
        ...
        output_pdf_path = f'PDF_FIRMADOS/{company}/{dni}/FIRMADO({validador})_{date_str}_{nombre_archivo}'
        rutaRetorno = f'{company}/{dni}/FIRMADO({validador})_{date_str}_{nombre_archivo}'
        ...
        respMsg = f"El archivo {output_p12_path} no se encontró. {type(e)} | ERROR: " + str(e)
        respStatus = False        
        return respStatus, respMsg        

    except IOError as e:
        respMsg = f"Error de E/S al leer el archivo: {type(e)} | ERROR: " + str(e)
        respStatus = False        
        return respStatus, respMsg        

    except ValueError:        
        respMsg = f"La contraseña del archivo P12 no es correcta. {type(e)} | ERROR: " + str(e)
        respStatus = False        
        return respStatus, respMsg

    except Exception as e:
        logging.info(f"Capturada excepción de tipo "+ str(e))
